    ...
    *※ 왼쪽 사이드바에 있는 것을 조건에 맞게 선택하신 후 조회버튼을 눌러주세요※*
    ...
    if st.sidebar.button('조회'):
        ...
        if '모두' in type_select:
            ...
            st.write('RENT_GTN type is ', type(data['RENT_GTN']))
            st.write(rent_gtn_select[0])
            st.write('rent_gtn_select[0] type is ', type(rent_gtn_select[0]))
        ...
        rent_fee_search = (data['RENT_FEE'] >= rent_fee_select[0]) & (data['RENT_GTN'] <= rent_fee_select[0])
